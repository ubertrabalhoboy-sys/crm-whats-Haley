...
export const transcriptScenarios = [
    ...
    },
    {
        ...
        source: "production_regression",
        validates: ["usa_midia_certa", "segue_ordem_comercial"],
        steps: [{ role: "assistant", text: "Confira nossas opcoes:" }],
        run() {
            assert.deepEqual(
                detectStructuredReplyIntent({
                    text: this.steps[0].text,
    ...
    },
    {
        ...
        source: "grounding_contract",
        validates: ["nao_inventa_valor", "mantem_grounding"],
        steps: [{ role: "assistant", text: "Seu frete ficou em R$ 8,50." }],
        run() {
            assert.deepEqual(
    ...
    },
    {
        ...
        source: "grounding_contract",
        validates: ["nao_inventa_valor", "mantem_grounding"],
        steps: [{ role: "assistant", text: "O pagamento via PIX ficou em R$ 42,00." }],
        run() {
            assert.deepEqual(
                detectUnverifiedCommercialClaim(this.steps[0].text, {
                    hasFreightCalculation: true,
    ...
    },
    {
        ...
        source: "logistics_contract",
        validates: ["nao_pula_etapa", "mantem_fluxo_logistico"],
        steps: [{ role: "assistant", text: "Como prefere pagar? PIX, Dinheiro ou Cartao?" }],
        run() {
            assert.equal(
                detectStructuredReplyIntent({
                    text: this.steps[0].text,
    ...
    },
    {
        ...
        source: "production_regression",
        validates: ["mantem_contingencia_operacional"],
        steps: [{ role: "assistant", text: "Como prefere pagar?" }],
        run() {
            const metrics = createAiTurnMetrics();
            markIterationStarted(metrics);
            markToolCompleted(metrics, {
...
export function runTranscriptBenchmark(options = {}) {
    const {
        print = true,
        failOnError = true,
    } = options;
    ...
    const overallScore =
        totalCriteriaChecks > 0
            ? Number(((totalCriteriaPasses / totalCriteriaChecks) * 100).toFixed(1))
            : 0;

    const criterionResults = [...scoreByCriterion.entries()]
        ...
        .map(([criterion, result]) => ({
            ...
            percent: result.total > 0
                ? Number(((result.passed / result.total) * 100).toFixed(1))
                : 0,
        }));

    if (print) {
        console.log(
            `AI transcript scenarios passed (${transcriptScenarios.length} casos) - dataset ${AI_TRANSCRIPT_DATASET_VERSION}`
        );
        console.log(`AI transcript benchmark score: ${overallScore.toFixed(1)}%`);
        for (const result of criterionResults) {
            console.log(
                `- ${toTitle(result.criterion)}: ${result.passed}/${result.total} (${result.percent.toFixed(1)}%)`
            );
        }
    ...
    return {
        datasetVersion: AI_TRANSCRIPT_DATASET_VERSION,
        scenarioCount: transcriptScenarios.length,
        overallScore,
        failures,
        criterionResults,
    };
