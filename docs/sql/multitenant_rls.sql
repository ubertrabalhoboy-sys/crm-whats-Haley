...
-- IMPORTANT: replace the placeholder UUID in the backfill block before running.

begin;

-- 1) Ensure tenant key columns exist
...
-- 1.a) Ensure FK constraints to restaurants
do $$
begin
  if not exists (
    select 1 from pg_constraint where conname = 'chats_restaurant_id_fkey'
  ) then
  ...
  if not exists (
    select 1 from pg_constraint where conname = 'messages_restaurant_id_fkey'
  ) then
  ...
  if not exists (
    select 1 from pg_constraint where conname = 'contacts_restaurant_id_fkey'
  ) then
...
-- 2) Backfill safe (replace UUID below with your restaurant_id)
with target as (
  select '00000000-0000-0000-0000-000000000000'::uuid as restaurant_id
)
update public.chats c
set restaurant_id = (select restaurant_id from target)
...
set restaurant_id = coalesce(
  m.restaurant_id,
  c.restaurant_id,
  (select restaurant_id from target)
)
from public.chats c
...
drop policy if exists restaurants_select_by_profile on public.restaurants;
create policy restaurants_select_by_profile
